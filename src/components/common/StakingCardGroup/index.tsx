...
const StakingCardGroup: React.FC<StakingCardProps> = ({ items, setModalOpen, setModalInfo }) => {
    ...
    const handleExtendClick = (apy: number, rewardRate: number, extendPercents: number[]) => {
        const info = {apy, rewardRate, extendPercents};
        setModalInfo(info);
        setModalOpen(true);
    }

    const handleClaimClick = async (index: number, apy: number) => {
        try {
                                                    ...
                                                    <div className={`connect-wallet-wrapper h-[54px] mt-4 w-full relative ${isWithdrawValid ? "bg-gradient" : "bg-[#212121]"}`}>
                                                        <div className={`connect-wallet-container absolute ${isWithdrawValid ? "bg-white" : "bg-[#444444]"}`}>
                                                            <button 
                                                                ...
                                                                onClick={() => {handleExtendClick(item.apy, item.rewardRate, extendPercentsOf(item.apy))}}
                                                            >
                                                                Extend {!isWithdrawValid && formatTime(remainingTime)}
                                                            </button>
                                                        </div>
                                                    </div>
                                                }
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <button
                                                            ...
                                                            onClick={() => handleClaimClick(index, item.apy)}
                                                        >
                                                            Claim {!isWithdrawValid && formatTime(remainingTime)} {loading[index] && <Spinner size={20} margin={12} />}
                                                        </button>
                                                    </div>
                                                </div>
                                            </>
